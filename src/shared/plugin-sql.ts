...
export function pluginIdentifier(id: string, name: string): string {
  if (!LOGICAL_RE.test(name))
    throw new Error(`invalid logical identifier: ${name}`);
...
export function formatPluginSql(
  ...
  for (const object of known) {
    if (!LOGICAL_RE.test(object))
      throw new Error(`invalid logical identifier: ${object}`);
  }
  let out = '';
  let i = 0;
  let state:
  ...
  while (i < sql.length) {
    ...
    if (state === 'code') {
      ...
      if (c === '{' || c === '}') {
        ...
        if (!LOGICAL_RE.test(name) || !known.has(name))
          throw new Error(`unknown SQL marker: ${name}`);
