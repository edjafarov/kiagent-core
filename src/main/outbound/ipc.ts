...
export function registerOutboundIpc(deps: {
  ...
  // list: `{ limit?: number }` is the only ALL-optional req in `Invokes`, and
  // the contextBridge surface (`window.kiagent.invoke`, preload.ts:11-16)
  // passes whatever it is handed straight through — types are erased there,
  // so a payload-less call reaches `ipcMain.handle`'s `fn(req)` as
  // `undefined` and would TypeError on the panel's PRIMARY read. Every other
  // destructured handler has a required payload, so there is no precedent
  // telling a caller the `{}` is mandatory.
  handle('outbox:list', async (req) => {
