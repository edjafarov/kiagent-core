...
Operators (gmail-style, inside the query string): \`from:\` \`to:\` \`participant:\` match people by case-insensitive substring on name or address — \`from:sebastian\`, \`from:@zoolatech.com\`, \`from:"Roman Kaplun"\`. Case folding is ASCII-only — for non-ASCII names (Ünal, MÜLLER) match the stored casing's lowercase form or use the address instead. \`label:inbox\`; \`has:attachment\`; \`filename:report\`; \`ext:pdf\`; \`in:gmail\` (alias \`source:\`) and \`type:email.thread\` mirror the JSON params; \`order:newest\`/\`order:relevance\` picks the sort (default: relevance with text, newest without). Repeat an operator to OR within it (\`from:a from:b\`); different operators AND. When a source has several connected accounts, merged results are ordered by date — order:relevance applies within each account. Example: \`from:@zoolatech.com has:attachment order:newest log*\`.
