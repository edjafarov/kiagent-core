...
describe('source proxy ↔ real child runtime', () => {
  ...
  it('auth verb "valueOf" is rejected cleanly instead of returning the channel unserializably (F5)', async () => {
    const { source, proxySet } = await setup();
    const auth = { prompt: jest.fn(), status: jest.fn(), oauth: jest.fn(), showQr: jest.fn() } as never as AuthChannel;
    ...
    await expect(proxySet.handleCall('auth', 'valueOf', [1])).rejects.toThrow('unknown auth verb valueOf');
  });
});
