...
describe('isAutomatedMessage', () => {
  ...
  it('flags bulk/list precedence', () => {
    expect(
      isAutomatedMessage({ precedence: 'bulk' }, 'a@example.com').matched,
    ).toBe(true);
    expect(
      isAutomatedMessage({ precedence: 'list' }, 'a@example.com').matched,
    ).toBe(true);
  });

  it('flags list headers', () => {
    expect(
      isAutomatedMessage({ 'list-unsubscribe': '<mailto:x>' }, 'a@example.com')
        .matched,
    ).toBe(true);
  });
  ...
  it('flags system-sender local parts', () => {
    ...
    expect(isAutomatedMessage({}, 'mailer-daemon@example.com').matched).toBe(
      true,
    );
    expect(
      isAutomatedMessage({}, 'Support <bounce+123@example.com>').matched,
    ).toBe(true);
  });

  it('flags empty Return-Path (bounce)', () => {
    expect(
      isAutomatedMessage({ 'return-path': '<>' }, 'a@example.com').matched,
    ).toBe(true);
  });
