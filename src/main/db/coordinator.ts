...
type Active = { owner: DbOwner; token: TxToken; rollback?: () => Promise<unknown> | unknown; expires: number };
...
export function createDbCoordinator(options: { leaseMs?: number } = {}): DbCoordinator {
    ...
    try {
      for (;;) {
        ...
        if (active && Date.now() > active.expires) {
          ...
          if (expired.rollback) { try { await expired.rollback(); } catch { /* connection owner handles fatal rollback */ } }
        }
  ...
  return {
    ...
    begin: async (owner, work, rollback) => {
      ...
      await enqueue(owner, undefined, async () => { active = { owner, token, rollback, expires: Date.now() + leaseMs }; await work(); return undefined; });
      return token;
    },
    finish: async (owner, token, work) => {
      ...
      try { return await enqueue(owner, token, work); } finally { active = undefined; void pump(); }
    },
    release: async (owner) => {
      if (active && sameOwner(owner, active.owner)) { const rollback = active.rollback; active = undefined; if (rollback) await rollback(); }
    ...
    close: async () => { closed = true; const rollback = active?.rollback; active = undefined; if (rollback) await rollback(); while (queue.length) queue.shift()!.reject(error('database coordinator is closed', 'DB_COORDINATOR_CLOSED')); },
