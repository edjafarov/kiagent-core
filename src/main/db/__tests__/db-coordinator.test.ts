...
describe('shared database coordinator', () => {
  ...
  it('admits a foreign begin after queued core work and the active owner commits', async () => {
    ...
    const nativeBeginB = jest.fn(async () => {
      order.push('b-native-begin');
    });
  ...
  it('serializes independently constructed adapters that share one owner handle', async () => {
    const coordinator = createDbCoordinator();
    const order: string[] = [];
    const ownerFromFirstAdapter = {
      kind: 'plugin' as const,
      extensionId: 'one',
      handle: 'shared',
    };
    const ownerFromSecondAdapter = {
      kind: 'plugin' as const,
      extensionId: 'one',
      handle: 'shared',
    };
    const makeAdapter = (
      owner: typeof ownerFromFirstAdapter,
      label: string,
    ) => ({
      begin: () =>
        coordinator.begin(owner, async () => {
          order.push(`${label}:begin`);
        }),
      commit: (token: string) =>
        coordinator.finish(owner, token, async () => {
          order.push(`${label}:commit`);
        }),
    });
  ...
  it('queues a second begin while the first native begin is still awaiting admission', async () => {
    ...
    const firstBeginGate = new Promise<void>((resolve) => {
      releaseFirstBegin = resolve;
    });
    let firstBeginStarted!: () => void;
    const firstBeginEntered = new Promise<void>((resolve) => {
      firstBeginStarted = resolve;
    });
    const secondNativeBegin = jest.fn(async () => {
    ...
    await coordinator.finish(ownerA, firstToken, async () => {
      order.push('first-commit');
    });
    const secondToken = await secondTokenPromise;
    await coordinator.finish(ownerB, secondToken, async () => {
  ...
  it('rejects queued begins on close without starting native work', async () => {
    ...
    expect(nativeBegin).not.toHaveBeenCalled();
    await expect(
      coordinator.finish(ownerA, tokenA, async () => undefined),
    ).rejects.toMatchObject({ code: 'DB_TX_TOKEN_INVALID' });
  });
  ...
  it('poisons queued begins for an owner after rollback failure before foreign work', async () => {
    ...
    const foreignWork = coordinator.run(
      good,
      undefined,
      async () => 'foreign-ok',
    );
    ...
    await expect(poisonedBegin).rejects.toMatchObject({
      code: 'DB_OWNER_POISONED',
    });
  ...
  it('rejects a token belonging to another owner without ending the transaction', async () => {
    ...
    await expect(
      coordinator.run(two, token, async () => 1),
  ...
  it('rolls back an expired transaction and admits the next owner', async () => {
    ...
    await expect(
      coordinator.run(two, undefined, async () => 'ok'),
    ).resolves.toBe('ok');
    expect(rolledBack).toBe(true);
    await coordinator.close();
  });
  ...
  it('treats a reactivated owner handle as distinct from its stale incarnation', async () => {
    ...
    await expect(
      coordinator.finish(oldOwner, token, async () => undefined),
  ...
  it('poisons an owner after rollback failure while allowing another owner to proceed', async () => {
    ...
    await expect(coordinator.release(bad)).rejects.toThrow('rollback failed');
    await expect(
      coordinator.run(bad, token, async () => 1),
    ).rejects.toMatchObject({ code: 'DB_OWNER_POISONED' });
    await expect(coordinator.run(good, undefined, async () => 2)).resolves.toBe(
      2,
    );
    await coordinator.close();
  });
  ...
  it('removes coordinator abort listeners whenever queued jobs settle', async () => {
    const queueScenario = async (
      work: () => unknown,
      settle: (
        coordinator: ReturnType<typeof createDbCoordinator>,
        owner: { kind: 'plugin'; extensionId: string },
        token: string,
    ...
    await queueScenario(
      () => 1,
      async (coordinator, owner, token, waiting) => {
    ...
    await queueScenario(
      () => 1,
      async (coordinator, owner, _token, waiting, controller) => {
        controller.abort();
        await expect(waiting).rejects.toMatchObject({
          code: 'DB_OPERATION_CANCELLED',
        });
    ...
    await queueScenario(
      () => 1,
      async (coordinator, owner, _token, waiting) => {
        await coordinator.release(owner);
        await expect(waiting).resolves.toBe(1);
      },
    );
    await queueScenario(
      () => 1,
      async (coordinator, _owner, _token, waiting) => {
        await coordinator.close();
        await expect(waiting).rejects.toMatchObject({
          code: 'DB_COORDINATOR_CLOSED',
        });
