...
describe('vision-helper driver (execFile protocol)', () => {
  it('spawns the binary with ["ocr", <image path>] and resolves with OCR text', async () => {
    ...
    const exec: ExecFileFn = (file, args, _opts, callback) => {
      ...
      seenArgs = args;
      imagePath = args[1];
      callback(null, JSON.stringify({ text: 'hello world', width: 10, height: 10, confidence: 0.9 }), '');
    };
    const helper = makeVisionHelper('/opt/kia-vision', noop, { execFileFn: exec });
  ...
  it('rejects with the stderr message on non-zero exit, and cleans up', async () => {
    ...
    await expect(helper.rasterizePdf(new Uint8Array([1, 2]), 3)).rejects.toThrow(
      /kia-vision rasterize failed: no such helper mode/,
    );
