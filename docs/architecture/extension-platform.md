...
| ----------- | ------------------------ | ------------------------------------------------------------------------------------------------------------ |
| `query`     | `host.query`             | Read the corpus: `document`, `children`, `byExternalId`, `search`, `count`, `countBy`, `accounts`            |
...
| `inference` | `host.inference`         | Local AI: `complete`, `see`, `read`, `hear`, plus `lane()` (the background lane's current `LaneState`, so a caller can wait for `'open'` instead of hammering a closed lane) and `describe(kind)` (the provider that would answer `kind` right now — `{ providerId, modelId, generation }`, or `null` when none is ready). `complete`'s widened opts: `profile: 'deterministic'` (temperature 0, seed 0, `maxTokens` required and capped at 512) gives classification-style callers repeatable output — checked by hand, not in CI: 20 runs of one prompt against one model produce identical text on one machine; bitwise determinism across hardware is NOT promised, repeatability comes from the caller's own persisted result, not the sampler — `system` (a separate system message, not concatenated into the prompt), and `generation` (the token `describe()` returned). A `lane: 'background'` call fails fast with `LaneClosedError` while the processing window is closed, exactly like a core worker; passing a stale `generation` to `complete()` fails with `ModelChangedError` (`{ expected, actual, modelId, source: 'generation' \| 'model-drift' }`) before any request reaches the model. **Both errors cross the extension RPC boundary by `name`, never `instanceof`** — class identity does not survive the fork (`src/shared/extension-rpc.ts`), so a caller discriminates on `err.name` plus, for `ModelChangedError`, its plain `expected`/`actual`/`modelId`/`source` fields. **Caching rationale**: a persisted result cache should be keyed on the canonical effective request plus the `modelId` obtained from `describe()` _before_ the lookup; the `generation` token turns a model switch between that lookup and the eventual `complete()` call into a rejected call the caller repeats, rather than a silently wrong cache hit. The richer `completeWithMeta()` (`{ text, providerId, modelId, generation, profile, promptTokens, completionTokens, truncated }`) exists on the plane (`src/main/core/inference.ts`) but is deliberately NOT lifted onto the extension surface in this wave. `complete()` resolves to the completion TEXT and echoes nothing back, so an extension learns the model identity from `describe()` and then *proves* it still held at call time by passing that `generation` to `complete()` — a call that resolves is itself the assertion, enforced twice (the plane's generation check before the provider is called, and the provider's own `expectModelId` backstop). What an extension therefore cannot see is the rest of the meta: `providerId`, the resolved `modelId`, `profile`, token usage, and — the one that matters for a persisted cache — `truncated`, so a ceiling-truncated answer is indistinguishable from a complete one. The `platform.lane` event (delivered via the `events` cap, payload `{ state: LaneState }`) fires on every real transition — **and once more, unconditionally, the first time either of its two triggers runs**, since its dedup state starts `null`: that first delivery announces the lane's already-current state rather than reporting a transition, which is correct and deliberate, not a bug. |
...
| Cap                  | Grants                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| -------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `send`               | Registration of the extension's `contributes.senders` as outbound transports. Main calls _in_ (`callSender`), past a confirmation gate; the extension gets no new surface. Bundled senders shadow it. An extension sender's auth-failure `Error.message` ending `reconnect the account in Settings` is classified as retryable-auth by the app's outbound error copy (`src/main/outbound/error-copy.ts`) — a phrasing contract shipped connectors depend on. |
